...
do iz = 1,nz 
    ...
    if (iz == 1) then 
        ...
        amx(row,row) = (&
            ...
            + adf(iz)*dwn(iz)*(sporo(iz)*w(iz)*1d0-sporo(iz)*w(iz)*1d0)/dz(iz)  &
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*1d0-sporo(iz-1)*w(iz-1)*0d0)/dz(iz)  &
            + sporo(iz)*kom(iz)   &
            )
        amx(row,row-nsp) = ( &
            ! - (0d0 &
            ...
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*0d0-sporo(iz-1)*w(iz-1)*1d0)/dz(iz)  &
            )
    else 
        ymx(row) = 0d0  &
...
do iz = 1,nz 
    ...
    if (iz == 1) then 
        ...
        omadv = omadv + adf(iz)*up(iz)*(sporo(iz)*w(iz)*omx(iz)-sporo(iz-1)*w(iz-1)*omx(iz-1))/dz(iz)*dz(iz) &
            + adf(iz)*dwn(iz)*(sporo(iz)*w(iz)*omx(iz)-sporo(iz)*w(iz)*omx(iz))/dz(iz)*dz(iz) &
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*omx(iz)-sporo(iz-1)*w(iz-1)*omx(iz-1))/dz(iz)*dz(iz) 
...
do iz = 1,nz 
    ...
    if (iz == 1) then 
        ymx(row) = &
            ...
            ! - 0d0  &  !  no bioturbation loss at the top boundary  
            ! )/dz(1)  &
            + adf(iz)*up(iz)*(sporo(iz)*w(iz)*ccx(iz)-0d0)/dz(1)  &
            + adf(iz)*dwn(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-sporo(iz)*w(iz)*ccx(iz))/dz(1)  &
            + adf(iz)*cnr(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-0d0)/dz(1)  &
            + sporo(iz)*rcc(iz)
        amx(row,row) = (&
            + sporo(iz)*(1d0)/dt &
        ...
        ymx(row) = & 
            ...
            + adf(iz)*up(iz)*(sporo(iz)*w(iz)*ccx(iz)-sporo(iz-1)*w(iz-1)*ccx(iz-1))/dz(iz)  &
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*ccx(iz)-sporo(iz-1)*w(iz-1)*ccx(iz-1))/dz(iz)  &
            + sporo(iz)*rcc(iz)
        amx(row,row) = (&
            + sporo(iz)*(1d0)/dt &
            ! - (0d0 &
            ...
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*1d0-0d0)/dz(iz)  &
            + sporo(iz)*drcc_dcc(iz)   &
            )*ccx(iz)
        amx(row,row-nsp) = ( &
            ! - (0d0 &
        ...
        ymx(row) = & 
            ...
            ! - (dbio(iz)+dbio(iz-1))*0.5d0*(ccx(iz)-ccx(iz-1))/(0.5d0*(dz(iz)+dz(iz-1))))/dz(iz)  & 
            + adf(iz)*up(iz)*(sporo(iz)*w(iz)*ccx(iz)-sporo(Iz-1)*w(iz-1)*ccx(iz-1))/dz(iz)  &
            + adf(iz)*dwn(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-sporo(Iz)*w(iz)*ccx(iz))/dz(iz)  &
            + adf(iz)*cnr(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-sporo(Iz-1)*w(iz-1)*ccx(iz-1))/dz(iz)  &
            + sporo(iz)*rcc(iz)
        amx(row,row) = (&
            + sporo(iz)*(1d0)/dt &
            ! - ((dbio(iz+1)+dbio(iz))*0.5d0*(-1d0)/(0.5d0*(dz(iz+1)+dz(iz))) &
...
do iz = 1,nz 
    ...
    if (iz == 1) then 
        ...
        ccadv = ccadv + adf(iz)*up(iz)*(sporo(iz)*w(iz)*ccx(iz)-0d0)/dz(1) * dz(iz) &
            + adf(iz)*dwn(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-sporo(iz)*w(iz)*ccx(iz))/dz(1) * dz(iz)  &
            + adf(iz)*cnr(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-0d0)/dz(1) * dz(iz)
        ...
        ccadv = ccadv + adf(iz)*up(iz)*(sporo(iz)*w(iz)*ccx(iz)-sporo(iz-1)*w(iz-1)*ccx(iz-1))/dz(iz) * dz(iz)  &
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*ccx(iz)-sporo(iz-1)*w(iz-1)*ccx(iz-1))/dz(iz) * dz(iz)  
        ! print*,iz, (1d0-poro(iz))*(ccx(iz)-cc(iz))/dt *dz(iz) &
            ! + (1d0-poro(Iz))*rcc(iz) *dz(iz) &
            ! - (1d0-poro(iz))*(0d0  &
            ! - (dbio(iz)+dbio(iz-1))*0.5d0*(ccx(iz)-ccx(iz-1))/(0.5d0*(dz(iz)+dz(iz-1))) )/dz(iz)* dz(iz) &
        ...
        ! ccdif = ccdif &
            ...
            ! - (dbio(iz)+dbio(iz-1))*0.5d0*(ccx(iz)-ccx(iz-1))/(0.5d0*(dz(iz)+dz(iz-1))) )/dz(iz)* dz(iz)
        ccadv = ccadv + adf(iz)*up(iz)*(sporo(iz)*w(iz)*ccx(iz)-sporo(iz-1)*w(iz-1)*ccx(iz-1))/dz(iz) * dz(iz)  &
            + adf(iz)*dwn(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-sporo(iz)*w(iz)*ccx(iz))/dz(iz) * dz(iz)  &
            + adf(iz)*cnr(iz)*(sporo(iz+1)*w(iz+1)*ccx(iz+1)-sporo(iz-1)*w(iz-1)*ccx(iz-1))/dz(iz) * dz(iz)  
        ! print*,iz,(1d0-poro(iz))*(ccx(iz)-cc(iz))/dt *dz(iz) &
            ! + (1d0-poro(Iz))*rcc(iz) *dz(iz)  &
            ! - (1d0-poro(iz))*((dbio(iz+1)+dbio(iz))*0.5d0*(ccx(iz+1)-ccx(iz))/(0.5d0*(dz(iz+1)+dz(iz))) &
            ! - (dbio(iz)+dbio(iz-1))*0.5d0*(ccx(iz)-ccx(iz-1))/(0.5d0*(dz(iz)+dz(iz-1))) )/dz(iz)* dz(iz) &
    ...
    if (nonlocal) then 
        do iiz = 1, nz
            if (trans(iiz,iz)==0d0) cycle
            ccdif = ccdif -trans(iiz,iz)/dz(iz)*(1d0-poro(iz))*dz(iiz)/(1d0-poro(iiz))*dz(iz)*ccx(iiz)
        enddo
    else 
        do iiz = 1, nz
            if (trans(iiz,iz)==0d0) cycle
    ...
    if (nonlocal) then 
        do iiz = 1, nz
            if (trans(iiz,iz)==0d0) cycle
            dw(iz) = dw(iz) - mvcc*(-trans(iiz,iz)/dz(iz)*(1d0-poro(iz))*dz(iiz)/(1d0-poro(iiz))*ccx(iiz))
        enddo
    endif
enddo
...
do iz = 1,nz 
    ...
    if (iz == 1) then
        ...
        ptadv = ptadv &
            ...
            + adf(iz)*cnr(iz)*(sporo(iz)*w(iz)*ptx(iz)-sporo(iz-1)*w(iz-1)*ptx(iz-1))/dz(iz)*dz(iz)  
        ! do iiz = 1, nz
            ! if (iiz==iz) cycle
            ! ptdif = ptdif -trans(iz,iiz)*ptx(iiz)&
                ! *(1d0-poro(iz))*rhosed/msed*dz(iz)
...
write(file_tmp,*) 1d6*(co3i*1d3-co3sat), ccx(1)*mcc/rho(1)*100d0, frt(1)  &
    ,ccx(nz)*mcc/rho(nz)*100d0, frt(nz),ccx(izml)*mcc/rho(izml)*100d0, frt(izml)
close(file_tmp)

open(unit=file_tmp,file=trim(adjustl(workdir))//'ccbur_sense_'// &
    'cc-'//trim(adjustl(chr(1,4)))//'_rr-'//trim(adjustl(chr(2,4)))  &
    //'.txt',action='write',status='unknown',access='append') 
write(file_tmp,*) 1d6*(co3i*1d3-co3sat), 1d6*ccadv
close(file_tmp)
