classdef lysocline_exp
    ...
    methods(Static)
        ...
        function run_all_lysocline_exp()
            %        function run_lysocline_exp(cc_rain_flx_in, rainratio_in, dep_in, dt_in)
            %% do lysocline experiment as in manuscript
            ...
            nz = 25;
%            lysocline_exp.run_one_lysocline_exp(24e-6, rainratio(5), 0.24, dt, switch_oxonly(1));
                ...
                for j = 1:10
                    for k = 1:length(rainratio)
                        for l = 1:1 %length(switch_oxonly)
                            ...
                            dep = i*6.0/25
                            rr = rainratio(k)
                            lysocline_exp.run_one_lysocline_exp(cc_rain_flx, rainratio(k), dep, dt, switch_oxonly(l));
                        end
                    end
                end
            end
        ...
        function run_one_lysocline_exp(cc_rain_flx_in, rainratio_in, dep_in, dt_in, oxonly_in)
            ...
            if(~global_var.def_nonrec)
                ...
                file_sigmly = sprintf('./2305_lysocline/matlab_sigmly.txt');
                file_sigmlyid = fopen(file_sigmly,'wt');
                file_sigmlyd = sprintf('./2305_lysocline/matlab_sigmlyd.txt');
                file_sigmlydid = fopen(file_sigmlyd,'wt');
                file_sigbtm = sprintf('./2305_lysocline/matlab_sigbtm.txt');
                file_sigbtmid = fopen(file_sigbtm,'wt');
                file_bound = sprintf('./2305_lysocline/matlab_bound.txt');
                file_boundid = fopen(file_bound,'wt');
                file_frac = sprintf('./2305_lysocline/matlab_frac.txt');
            ...
            while 1     % "starting point" if calculation needs to be restarted with smaller timestep
                ...
                depf = dep_in;  % dep_max;   % max depth to be changed to - here no depth change
                ...
                while(2>1)              %   Here change to unknwon iteration
                    ...
                    if(global_var.def_sense)    % no signal tracking
                        if(flag_steadystate) % solid volume is in steady-state -> stop overall iteration
                            break;
                        end
            ...
            end   %  start all over with smaller timestep???
            ...
            if(oxonly_in)
                str_lys = sprintf('./2305_lysocline/lys_sense_cc-%2.1e_rr-%.2f_oxonly.txt',cc_rain_flx_in, rainratio_in);
                str_ccbur = sprintf('./2305_lysocline/ccbur_sense_cc-%2.1e_rr-%.2f_oxonly.txt',cc_rain_flx_in, rainratio_in);
            else
                str_lys = sprintf('./2305_lysocline/lys_sense_cc-%2.1e_rr-%.2f_oxanox.txt',cc_rain_flx_in, rainratio_in);
                str_ccbur = sprintf('./2305_lysocline/ccbur_sense_cc-%2.1e_rr-%.2f_oxanox.txt',cc_rain_flx_in, rainratio_in);
            end
